...
(defvar pukiwiki-fetch-index-regexp-1.4.7
 "<li><a[^?]*\\?\\([^\"]*\\)\">\\([^<]*\\)</a>[<small>]*\\([^<]*\\)[</small>]*</li>"
 "index として抽出する anchor の正規表現の Pukiwiki 1.4.7用。")
  ...
  (let (str)
    (save-match-data
      ...
      (cond
       ...
       ((string-match "1.4.7" str)
        (setq pukiwiki-1-3-p nil)
        (setq pukiwiki-fetch-index-regexp pukiwiki-fetch-index-regexp-1.4.7))
       (t
        (setq pukiwiki-1-3-p nil)
      ...
      (while (setq pos
        ...
        (setq result
              (replace-match
               ...
               t t result)))
      (setq result (url-unhex-string str)) ; 日本語ページを編集するための処理追加
